...
export default function CirclePage() {
  ...
  const add = () => {
    ...
    if (entries.length >= 5) { setError("Keep your circle to five steady entries. Remove one to add another."); return; }
    setError("");
    setEntries((es) => [...es, { id: uid("c"), ...form, label: form.label.trim() }]);
    setForm({ label: "", kind: "Friend", supportReceived: "", supportGiven: "", lastConnection: "", nextIntention: "" });
  };
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-12">
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-2">
        <Card>
          <h2 className="font-bold">Add to your circle ({entries.length}/5)</h2>
        ...
        <div>
          ...
          : <ul className="grid gap-3">{entries.map((c) => (
            <li key={c.id} className="royal-card p-5">
              ...
              <button className="mt-2 text-sm text-error underline" onClick={() => { if (confirm("Remove this entry?")) setEntries((es) => es.filter((x) => x.id !== c.id)); }}>Remove</button>
            </li>))}</ul>}
        </div>
      </div>
    </div>
